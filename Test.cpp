...
TEST_CASE("add father")
{
   ...
   CHECK((M->mother->mother->name).compare("Michal") == 0);
   CHECK((M->mother->mother->mother->name).compare("Tehila") == 0);
}*/

TEST_CASE("check find")
{
   Tree T("Mika");
   T.addFather("Mika", "Yitzak").addFather("Yitzak", "Israel").addFather("Israel", "Avraham").addMother("Israel", "Pnina").addMother("Mika", "Stella").addMother("Stella", "Michal").addFather("Stella", "Yaakov").addMother("Yitzak", "Tamar").addFather("Tamar", "Meir").addMother("Tamar", "Hana");
   T.display();
   ...
   CHECK(T.find("great-grandfather") == string("Avraham")); //11
}
...
TEST_CASE("Tree 3")
{
   ...
   CHECK(T.relation("Niv") == string("grandfather"));//95
}

TEST_CASE("remove")
{
   Tree M("ran");
   M.addMother("ran", "isca").addFather("ran", "tal").addFather("isca", "moshe").addMother("isca", "ronit");
   M.remove("ran"); // removes Avraham and Terah
	CHECK(M.relation("ran").compare("unrelated"));
   CHECK(M.relation("isca").compare("unrelated"));
   CHECK(M.relation("tal").compare("unrelated"));
   CHECK(M.relation("moshe").compare("unrelated"));
   CHECK(M.relation("ronit").compare("unrelated"));
   CHECK(M.relation("Terah").compare("unrelated"));
}